...
export class AdminChartComponent implements OnInit {
  ...
  onSubmit()
  {
    ...
    let l: ReportParam = {start: this.dateForm.get("start")?.value, end:this.dateForm.get("end")?.value, email:""};
    this.rideService.getAdminDataForReport(l).subscribe((response) => {
